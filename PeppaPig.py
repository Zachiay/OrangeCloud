...
def main():
    ...
    if op==1 or op==2: # 本地对战
        if op==1:
            A_vs_B() # 人人对战
        else :
            player_vs_Peppa() # 人机对战
        ready()  # 玩家初始化
        Turn = 1
        Start()  # 游戏初始化并开始
        while True:
...
class User:
    ...
    def action(self):   # 玩家的行动
        ...
        if self.opponent > len(cardList)*3 +self.total:
    ...
    def enquire(self):
        ...
        if self.opponent > len(cardList)*3 +SAtotal+ self.total:
            self.winwin=1   # 必胜局面：对方手牌 > 我方手牌 +放置区剩余手牌 +未翻卡组数*3

    # 1.摸牌
    def draw(self):
        global card
        card = cardList.pop() # 牌组pop一张牌
        print(self.name, "摸了一张牌：\t\t                            ","".join(card))
        self.flower=card[0] # 标记此牌花色
    # 2.出牌
    def OutPoker(self):
        global card
...
class Peppa(User):
    ...
    '''
    # 游戏行动
    def action(self):   # 与User类中的AI_action托管功能相同
        print("---------------------------------------------------------该回合由",self.name,"出牌--------------")
        global card
        global Up_card
        global op
